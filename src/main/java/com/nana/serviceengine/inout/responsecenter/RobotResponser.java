...
public class RobotResponser {
	...
	public void responseMessage(String mes, UserMessage message) {
		if ("//TODO".equals(mes)) {
			String html = ZhidaoHtmlCreator.getInstance().createAppHtml(
					ZhidaoAnalysis.getInstance().getBeanFromHtml(
							message.getMessage()));
			if (html != null) {
				...
				RobotConsumerListener.getInstance().sendMsg(
						rpMessage.getRpMessage(), message.getReqMessage());
				return;
			}
			mes = "我听不懂您在说什么。";
		}
		RobotConsumerListener.getInstance().sendMsg(mes,
	 ...
	 * @param message
	 */
	public void responseMessage(ResponseMessageAdapter rpMessage,
			UserMessage message) {
		RobotConsumerListener.getInstance().sendMsg(rpMessage.getRpMessage(),
				message.getReqMessage());
	}

	/**
	 * 指定audio消息和display消息
	 * 以后如果参数增多，可能需要在程序中构造完，调用responseMessage(ResponseMessageAdapter
	 * rpMessage,UserMessage message) 发送消息
	 * 
	 * @param audioTxt
	 * @param disPlayTxt
	 * @param message
	 */
	public void responseMessage(String audioTxt, String disPlayTxt,
			UserMessage message) {
		ResponseMessageAdapter rpMessage = new ResponseMessageAdapter();
