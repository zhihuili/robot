...
package com.nana.robot.chatterbean.text;
...
public class Sentence {
	...
	public Sentence(String original, Integer[] mappings, String normalized) {
		...
		setSplittedOfSentence(normalized);
	}
	 ...
	 */
	public String[] getSplittedOfSentence() {
		return splitted;
	}
	...
	public String original(int beginIndex, int endIndex) {
		...
		value = value.replaceAll(
				"^[^A-Za-z0-9\u4E00-\u9FA5]+|[^A-Za-z0-9\u4E00-\u9FA5]+$", " ");// 这里修改了正则表达式，使它兼容中文字符。
		return value;
	}
	...
	public String toString() {
		...
		for (int i = 0; i < splitted.length; i++) {
			if (i == 0)
				output.append(splitted[i].replace(" ", "#"));
			else
				output.append("," + splitted[i].replace(" ", "#"));
		}
		output.append("]");
	...
	public void setSplittedOfSentence(String normalized) {
		this.normalized = normalized;
		// ##############
		if (normalized != null)
			splitted = normalized.trim().split(" ");
		// System.out.println("###splitted of sentence:");
